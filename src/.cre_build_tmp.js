...
var INITIAL_MINT_AMOUNT = 888000000n * 10n ** 18n;
var MAX_MINT_PER_TX = 1000000n * 10n ** 18n;
var SLURA_CHAIN_SELECTOR = 3034092155422581607n;
var onCronTrigger = async (runtime) => {
  ...
  const evm = new ClientCapability(SLURA_CHAIN_SELECTOR);
  const forwarder = runtime.config.forwarderAddress;
  const receiver = runtime.config.custodianAddress;
  const reserveCall = encodeCallMsg({
    from: zeroAddress,
    to: runtime.config.aggregatorAddress,
  ...
  const [roundId, answer] = decodeFunctionResult({ abi: AGGREGATOR_ABI, functionName: "getFullRoundData", data: bytesToHex(reserveReply.data) });
  ...
  const supplyCall = encodeCallMsg({
    from: zeroAddress,
    to: runtime.config.vezProxyAddress,
  ...
  const [totalSupply] = decodeFunctionResult({ abi: VEZ_PROXY_ABI, functionName: "totalSupply", data: bytesToHex(supplyReply.data) });
  runtime.log(`Total supply after mint: ${totalSupply}`);
  const balanceCall = encodeCallMsg({
    from: zeroAddress,
    to: runtime.config.vezProxyAddress,
    data: encodeFunctionData({ abi: VEZ_PROXY_ABI, functionName: "balanceOf", args: [runtime.config.initialRecipient] })
  });
  const balanceReply = evm.callContract(runtime, { call: balanceCall, blockNumber: LAST_FINALIZED_BLOCK_NUMBER }).result();
  const [balance] = decodeFunctionResult({ abi: VEZ_PROXY_ABI, functionName: "balanceOf", data: bytesToHex(balanceReply.data) });
